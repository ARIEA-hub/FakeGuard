...
def _post(path: str, json: Dict, params: Optional[Dict] = None) -> Optional[Dict]:
    try:
        ...
        st.error("❌ Cannot reach the API. Start it with:  `uvicorn src.app.main:app --reload`")
...
def render_result(result: Dict) -> None:
    ...
    with col_badge:
        ...
        st.markdown(
            f'<span class="{badge_cls}">{icon} {label}</span>', unsafe_allow_html=True
        )
        st.metric("Confidence", f"{confidence * 100:.1f}%")
        st.caption(f"Model: `{model}` · Text length: {result.get('input_text_length', '?')} chars")
    ...
    with col_chart:
        fig = go.Figure(
            go.Bar(
                ...
                text=[f"{v*100:.1f}%" for v in [probs.get("FAKE", 0), probs.get("REAL", 0)]],
                textposition="inside",
...
with st.sidebar:
    ...
    healthy = api_healthy()
    st.success("✅ API online") if healthy else st.error("❌ API offline")

    models = available_models()
    selected_model = (
        st.selectbox("Model", models, index=0) if models else "logistic_regression"
    )
    if not models:
...
with tab_article:
    ...
    with st.form("article_form"):
        ...
        with col_a:
            submitted = st.form_submit_button("🔎 Detect", type="primary", use_container_width=True)
        with col_b:
            compare_all = st.form_submit_button("⚖️ Compare all models", use_container_width=True)
    ...
    if (submitted or compare_all) and not body.strip():
        ...
        with st.spinner("Classifying…"):
            result = _post("/predict", {"title": headline, "text": body}, {"model_name": selected_model})
        if result:
            st.divider()
...
with tab_url:
    ...
    st.caption(
        "The system will scrape the article body automatically. "
        "Paywalled or bot-protected pages may fail gracefully."
    )

    with st.form("url_form"):
        url_input = st.text_input("Article URL", placeholder="https://www.bbc.com/news/…")
        url_submit = st.form_submit_button("🌐 Analyse URL", type="primary", use_container_width=True)

    if url_submit and not url_input.strip():
        st.warning("Please enter a URL.")
    elif url_submit:
        with st.spinner(f"Fetching and classifying: {url_input}…"):
            result = _post("/predict/url", {"url": url_input}, {"model_name": selected_model})
        if result:
            st.divider()
...
with tab_compare:
    ...
    if metrics_data:
        ...
        rows = [
            {"Model": name.replace("_", " ").title(), **{k: m.get(k, 0) for k in metric_cols}}
        ...
        st.info("No metrics found. Train the models first: `python scripts/run_training.py`")
...
with tab_analytics:
    ...
    pipeline_df = pd.DataFrame(
        {
            "Stage": ["Input", "Validation", "Cleaning", "Vectorisation", "Classification", "Response"],
            "Tool": ["Text / URL / Batch", "Pydantic v2", "NLTK · regex", "TF-IDF (scikit-learn)", "4 ML models", "FastAPI · Streamlit"],
            "Output": [
                "Raw article string",
                "Validated schema",
